...
module Moped
  ...
  class Connection
    ...
    def read_data(socket, length)
      ...
      begin
        ready = IO.select([socket], nil, [socket], @options[:op_timeout] || @options[:timeout])
        unless ready
          raise Errors::OperationTimeout.new("Took more than #{@options[:op_timeout]} seconds to receive data.")
        end
      rescue IOError => e
